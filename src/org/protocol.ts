...
const SECTION_RE = /^(DECISION|OPEN|ACTIONS|BLOCKERS|WATCH|VERDICT)\s*:(.*)$/i;
const BULLET_RE = /^-\s+(.*)$/;
...
export function parseCloseBlock(text: string): CloseBlock {
  const out: CloseBlock = { decisions: [], open: [], actions: [] };
  let section: 'decision' | 'open' | 'actions' | null = null;
