...
**최소 프로필이 필수입니다.** codex와 gemini가 사용자의 개인 개발 환경(스킬, MCP 서버 다수)을 기본 상속해, 한 줄 프롬프트에 21,068 토큰을 소비하고 기동 시 연결 실패 노이즈를 냅니다. `SeatSpec`에 최소 프로필(스킬 비활성, MCP 비활성, 고정 모델)을 넣고 좌석이 개인 환경을 상속하지 않도록 격리합니다. 예산이 프롬프트가 아니라 환경 때문에 소진되는 것을 막습니다.
