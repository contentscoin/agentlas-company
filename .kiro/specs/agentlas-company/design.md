...
### War Room 은 열린 채로 남습니다 (Task 6.1)

R3.7의 뒷문장은 "**오너만이 종결할 수 있게** 해야 한다"입니다. 소집은 `MeetingResult.status = 'war-room'`이라는 반환값이었고, 프로세스가 끝나면 사라졌습니다. 닫을 대상이 없으니 뒷문장은 절반도 구현되지 않았고, 원장에 소집 기록만 남은 채 회사는 아무 일 없다는 듯 다음 작업으로 갔습니다. 장부는 상태 루트 아래 `org/warrooms.json`(0600)입니다.

**자동 종결 경로를 두지 않습니다.** 만료도, 조건부 close도 없습니다. 시간이 지나서·다음 회의가 성공해서·재시도가 통과해서 저절로 닫히면 그것은 오너가 종결한 것이 아니고, R3.7이 막으려는 것이 정확히 그 자동 해소입니다 — Critic이 BLOCK한 사안이나 두 번 실패한 과제가 사람 눈을 거치지 않고 흘러가는 것.

**종결은 `누가`와 `왜`를 둘 다 요구합니다.** 종결자가 없으면 "오너만 닫는다"를 나중에 증명할 수 없고, 사유가 없으면 목록에서 지우는 것과 다를 바가 없습니다. 이미 닫힌 방은 다시 닫히지 않습니다 — 두 번째 종결이 첫 기록을 덮으면 실제로 판단한 사람이 지워집니다.

**세 경로가 같은 문턱을 씁니다.** 오피스 API(`POST /api/warrooms/:id/close`), 콘솔 War Room 탭, CLI(`company warroom close`) 모두 단계별 인증과 사유를 요구합니다. 한쪽이 느슨하면 느슨한 쪽이 실질 규칙이 되기 때문입니다.

**싼 검사를 인증보다 먼저 합니다.** 인증 코드는 한 번 쓰면 끝이므로, 사유를 빠뜨린 요청이 맞는 코드를 태우면 오너는 다음 창까지 기다려야 합니다. 실측에서 실제로 그 상태에 빠졌고, 같은 함정이 있던 승인 경로(digest 누락)도 함께 고쳤습니다.
