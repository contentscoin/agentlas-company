...
- [x] 18. 검증과 복기 (R11) — **태스크가 없던 요구사항**
  - **왜 새로 세웠나.** R11 은 요구사항에 있는데 tasks.md 에 태스크가 없었다.
    그 사이 Task 15 의 `retro` 스텝이 R11 을 기다리며 미구현으로 막혀 있었고,
    발행 게이트도 R11.5(BLOCK)를 반영하지 않고 있었다
