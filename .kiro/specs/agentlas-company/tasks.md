...
- [x] 15. 레시피와 무인 스케줄
  ...
  - [x] 15.2 `retro` 스텝 — Task 18(R11)에서 붙였다. 예측이 없으면 로딩에서
        거부한다 — 사후 소감은 복기가 아니다
  - [ ] 15.3 Studio 스텝 — 지금 발행 스텝은 `brandOk` 를 레시피에 적어 오너가
        브랜드 책임을 지는 방식이다. Studio 를 레시피 스텝으로 만들면 브랜드
        대조가 파이프라인 안에서 일어나고 그 임시 통로를 없앨 수 있다
  - _Requirements: R12_
...
- [x] 18. 검증과 복기 (R11) — **태스크가 없던 요구사항**
  ...
  - [ ] 18.3 Studio → 검증 연결 — `company assure` 는 파일을 받는다. Studio 산출물이
        자동으로 검증을 거쳐 발행으로 가는 경로는 아직 없다 (Task 15.3 과 같은 자리)
